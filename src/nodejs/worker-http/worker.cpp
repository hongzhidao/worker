...
Worker::Worker(napi_env env, napi_value jsthis):
    nxt_napi(env),
    wrapper_(wrap(jsthis, this, destroy)),
    unit_ctx_(nullptr)
{
    nxt_unit_debug(NULL, "Worker::Worker()");
}
...
napi_value
Worker::init(napi_env env, napi_value exports)
{
    nxt_napi    napi(env);
    napi_value  ctor;
    ...
    try {
        ctor = napi.define_class("Worker", create, 2, unit_props);
        constructor_ = napi.create_reference(ctor);

        napi.set_named_property(exports, "Worker", ctor);
...
void
Worker::destroy(napi_env env, void *nativeObject, void *finalize_hint)
{
    Worker  *obj = reinterpret_cast<Worker *>(nativeObject);
...
napi_value
Worker::create(napi_env env, napi_callback_info info)
{
    nxt_napi    napi(env);
    ...
    try {
        ...
        if (target != nullptr) {
            /* Invoked as constructor: `new Worker(...)`. */
            jsthis = napi.get_cb_info(info);

            new Worker(env, jsthis);
            napi.create_reference(jsthis);

            return jsthis;
        }

        /* Invoked as plain function `Worker(...)`, turn into construct call. */
...
napi_value
Worker::create_server(napi_env env, napi_callback_info info)
{
    Worker             *obj;
    size_t           argc;
    nxt_napi         napi(env);
...
failed:
    ...
    napi_throw_error(env, NULL, "Failed to create Worker object");
...
napi_value
Worker::listen(napi_env env, napi_callback_info info)
{
    return nullptr;
}
...
void
Worker::request_handler_cb(nxt_unit_request_info_t *req)
{
    Worker  *obj;

    obj = reinterpret_cast<Worker *>(req->unit->data);
...
void
Worker::request_handler(nxt_unit_request_info_t *req)
{
...
void
Worker::websocket_handler_cb(nxt_unit_websocket_frame_t *ws)
{
    Worker  *obj;

    obj = reinterpret_cast<Worker *>(ws->req->unit->data);
...
void
Worker::websocket_handler(nxt_unit_websocket_frame_t *ws)
{
...
void
Worker::close_handler_cb(nxt_unit_request_info_t *req)
{
    Worker  *obj;
...
void
Worker::close_handler(nxt_unit_request_info_t *req)
{
...
void
Worker::shm_ack_handler_cb(nxt_unit_ctx_t *ctx)
{
    Worker  *obj;

    obj = reinterpret_cast<Worker *>(ctx->unit->data);
...
void
Worker::shm_ack_handler(nxt_unit_ctx_t *ctx)
{
...
int
Worker::add_port(nxt_unit_ctx_t *ctx, nxt_unit_port_t *port)
{
    int          err;
    Worker         *obj;
    uv_loop_t    *loop;
...
void
Worker::remove_port(nxt_unit_t *unit, nxt_unit_ctx_t *ctx, nxt_unit_port_t *port)
{
    port_data_t  *data;
...
void
Worker::quit_cb(nxt_unit_ctx_t *ctx)
{
    Worker  *obj;
...
void
Worker::quit(nxt_unit_ctx_t *ctx)
{
    ...
    try {
        ...
        nxt_async_context   async_context(env(), "worker_quit");
        nxt_callback_scope  async_scope(async_context);
...
void
Worker::create_headers(nxt_unit_request_info_t *req, napi_value request)
{
    uint32_t            i;
...
inline void
Worker::append_header(nxt_unit_field_t *f, napi_value headers,
...
napi_value
Worker::create_socket(napi_value server_obj, nxt_unit_request_info_t *req)
{
...
napi_value
Worker::create_request(napi_value server_obj, napi_value socket,
...
napi_value
Worker::create_response(napi_value server_obj, napi_value request,
    nxt_unit_request_info_t *req)
{
    napi_value  constructor, res;
    req_data_t  *req_data;
...
napi_value
Worker::create_websocket_frame(napi_value server_obj,
...
napi_value
Worker::request_read(napi_env env, napi_callback_info info)
{
    void                     *data;
    uint32_t                 wm;
    nxt_napi                 napi(env);
...
napi_value
Worker::response_send_headers(napi_env env, napi_callback_info info)
{
    int                      ret;
    char                     *ptr, *name_ptr;
    bool                     is_array;
    ...
    try {
        ...
        /*
         * Each name and value are 0-terminated by libworker.
...
napi_value
Worker::response_write(napi_env env, napi_callback_info info)
{
    int                      ret;
    void                     *ptr;
...
napi_value
Worker::response_end(napi_env env, napi_callback_info info)
{
    nxt_napi                 napi(env);
    napi_value               this_arg;
    req_data_t               *req_data;
...
napi_value
Worker::websocket_send_frame(napi_env env, napi_callback_info info)
{
    int                      ret, iovec_len;
    bool                     fin;
    size_t                   buf_len;
...
napi_value
Worker::websocket_set_sock(napi_env env, napi_callback_info info)
{
    nxt_napi                 napi(env);
...
void
Worker::conn_destroy(napi_env env, void *r, void *finalize_hint)
{
...
void
Worker::sock_destroy(napi_env env, void *r, void *finalize_hint)
{
...
void
Worker::req_destroy(napi_env env, void *r, void *finalize_hint)
{
...
void
Worker::resp_destroy(napi_env env, void *r, void *finalize_hint)
{
