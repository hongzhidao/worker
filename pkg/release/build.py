...
def write_deb(bundle, destination, manifest, epoch):
    ...
    with tempfile.TemporaryDirectory(prefix='deb-', dir=destination) as temporary:
        ...
        (control / 'control').write_text(
            ...
            'Section: web\nPriority: optional\n'
            f'Depends: libc6 (>= {manifest["glibc_minimum"]}), coreutils, util-linux, sed\n'
        ...
        service.write_text(
            ...
            '[Service]\nType=simple\nDynamicUser=yes\n'
            f'StateDirectory={package}\nStateDirectoryMode=0700\n'
            f'CacheDirectory={package}\nRuntimeDirectory={package}\nRuntimeDirectoryMode=0700\n'
            f'Environment=WORKER_STATE=/var/lib/{package}/state\n'
            f'Environment=WORKER_CONTROL=/run/{package}/control.sock\n'
            f'Environment=WORKER_DATA_DIR=/var/lib/{package}/data\n'
        ...
        defaults.write_text(
            '# Initial application options only apply to empty state.\n'
            '# Set WORKER_CONFIG to apply a JSON configuration on every start.\n'
            '# WORKER_CONFIG=/etc/worker/config.json\n'
            '# WORKER_APP=/srv/myapp\n'
            '# WORKER_LANGUAGE=python\n'
            'WORKER_LISTEN=127.0.0.1:8080\n'
            '# WORKER_PHP_LISTEN=127.0.0.1:8081\n')
        (control / 'conffiles').write_text('/etc/default/' + package + '\n')
        for hook, command in (
            ('postinst', f'case "$1" in configure) systemctl daemon-reload; systemctl try-restart {package}.service || : ;; esac'),
            ('prerm', f'case "$1" in remove) systemctl stop {package}.service || : ;; esac'),
...
def main():
    ...
    parser.add_argument('flavor', choices=('python', 'php', 'all'))
    ...
    languages = ('python', 'php') if args.flavor == 'all' else (args.flavor,)
    release = version()
    ...
    with tempfile.TemporaryDirectory(prefix='release-', dir=build_root) as temporary:
        ...
        if len(modules) != len(languages):
            raise RuntimeError('unexpected number of language modules')
        for module in modules:
            copy(module, stage / 'modules' / module.name)
        dependencies = Dependencies(stage, php_prefix)
        runtime_versions = {}
        extensions = []
        for language in languages:
            runtime_builder = python_runtime if language == 'python' else php_runtime
            full_version, minor, language_extensions = runtime_builder(stage, dependencies)
            runtime_versions[language] = full_version
            extensions.extend(language_extensions)
        ...
        dependencies.collect([stage / 'bin' / 'workerd', stage / 'bin' / 'curl',
                              *(stage / 'modules' / module.name for module in modules)])
        ...
        runtime_name = 'all' if args.flavor == 'all' else args.flavor + minor
        name = f'worker-{runtime_name}-{release}-{distribution}-{architecture}'
        manifest = {
            'version': release, 'revision': revision,
            'dirty_source': dirty and not bool(args.source_ref), 'dirty_packager': dirty,
            'flavor': args.flavor, 'runtime_versions': runtime_versions,
        ...
        if args.flavor != 'all':
            manifest['runtime_version'] = full_version
        ...
        for language in languages:
            copy(ASSETS / 'runtime', stage / language)
            (stage / language).chmod(0o755)
        ...
        for language in languages:
            example = 'wsgi.py' if language == 'python' else 'index.php'
