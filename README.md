...
Each application's `responses` object contains cumulative `1xx`, `2xx`, `3xx`,
`4xx`, and `5xx` counts. A response is counted once when its final headers are
prepared for sending, including errors generated by Worker for that application.
The `1xx` bucket counts `101` protocol upgrades; temporary informational responses
such as `100 Continue` and `103 Early Hints` are not counted. Status codes outside
the `100`-`599` range are not bucketed.

These counts describe response statuses, not successful delivery of the full
response to the client. A later body error does not add another response. Requests
