...
def test_static_variables(temp_dir):
    ...
    assert client.get(url=temp_dir + '/assets/index.html')['status'] == 200

    assert 'success' in update_share(temp_dir + '/assets${uri}')
    assert client.get(url='/index.html')['status'] == 200
...
def test_static_variables_buildin_mid(temp_dir):
    assert 'success' in update_share(temp_dir + '$uri/index.html')
...
def test_static_variables_invalid(temp_dir):
    assert 'error' in update_share(temp_dir + '/assets/d$r$uri')
    assert 'error' in update_share(temp_dir + '/assets/$$uri')
