...
def test_routes_match_uri_positive():
    ...
    assert (
        client.get(url='/slash/foo/..?args')['status'] == 200
    ), 'dot dot args'
    assert (
        client.get(url='/slash/foo/..#frag')['status'] == 200
    ), 'dot dot frag'
    assert client.get(url='/slash/.')['status'] == 200, 'trailing dot'
    assert (
        client.get(url='/slash/foo/..')['status'] == 200
    ), 'trailing dot dot'
...
def test_routes_match_headers_array():
    ...
    def check_headers(hds):
        ...
        assert (
            client.get(headers=hds)['status'] == 200
        ), 'headers array match'
...
def test_routes_match_arguments_chars():
    ...
    def check_args(args, query):
        ...
        assert client.get(url='/?' + query)['status'] == 200
...
def test_routes_match_arguments_multiple():
    ...
    assert (
        client.get(url='/?foo=bar&blah=test')['status'] == 200
    ), 'multiple 2'
    assert client.get(url='/?foo=bar&blah')['status'] == 404, 'multiple 3'
    assert (
        client.get(url='/?foo=bar&blah=tes')['status'] == 404
    ), 'multiple 4'
    assert (
...
def test_routes_match_arguments_multiple_rules():
    ...
    assert (
        client.get(url='/?foo=blah&foo=bar&foo=')['status'] == 404
    ), 'rules 5'
...
def test_routes_source_port():
    ...
    route_match({"source": "127.0.0.1:" + str(port)})
    ...
    route_match({"source": ["*:" + str(port), "!127.0.0.1"]})
