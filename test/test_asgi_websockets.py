...
def test_asgi_websockets_handshake():
    ...
    assert resp['headers']['Sec-WebSocket-Accept'] == ws.accept(
        key
    ), 'key'
...
def test_asgi_websockets_length_long():
    ...
    ws.frame_write(
        sock, ws.OP_CONT, 'fragment2', length=2 ** 64 - 1
    )
...
def test_asgi_websockets_handshake_upgrade_absent():  # FAIL https://tools.ietf.org/html/rfc6455#section-4.2.1
    client.load('websockets/mirror')
...
def test_asgi_websockets_2_10__2_11():
    ...
    for i in range(0, 10):
        ws.frame_write(sock, ws.OP_PING, 'payload-%d' % i)

    for i in range(0, 10):
        frame = ws.frame_read(sock)
        check_frame(frame, True, ws.OP_PONG, 'payload-%d' % i)

    # 2_11

    for i in range(0, 10):
        opcode = ws.OP_PING
        ws.frame_write(sock, opcode, 'payload-%d' % i, chopsize=1)
...
def test_asgi_websockets_3_1__3_7():
    ...
    ws.frame_write(
        sock, ws.OP_TEXT, payload, rsv1=True, rsv2=True
    )
    ...
    ws.frame_write(
        sock, ws.OP_TEXT, payload, rsv3=True, chopsize=1
    )
    ws.frame_write(sock, ws.OP_PING, '')
    ...
    ws.frame_write(
        sock, ws.OP_PING, payload, rsv2=True, rsv3=True
    )
    ...
    ws.frame_write(
        sock, ws.OP_CLOSE, payload, rsv1=True, rsv2=True, rsv3=True
    )
...
def test_asgi_websockets_5_1__5_20():
    ...
    ws.frame_write(
        sock, ws.OP_TEXT, 'fragment1', fin=False, chopsize=1
    )
    ws.frame_write(
        sock, ws.OP_CONT, 'fragment2', fin=True, chopsize=1
    )
    ...
    ws.frame_write(
        sock, ws.OP_CONT, 'non-continuation payload', fin=True
    )
    ...
    ws.frame_write(
        sock, ws.OP_TEXT, 'Hello, world!', fin=True, chopsize=1
    )
    check_close(sock, 1002)
    ...
    ws.frame_write(
        sock, ws.OP_CONT, 'non-continuation payload', fin=False
    )
    ws.frame_write(sock, ws.OP_TEXT, 'Hello, world!', fin=True)
    check_close(sock, 1002)
    ...
    if frame['opcode'] == ws.OP_TEXT:
        check_frame(
            frame, True, ws.OP_TEXT, 'fragment1fragment2'
        )
        frame = None
