...
class Log:
    ...
    @staticmethod
    def findall(pattern, name=WORKER_LOG, flags=re.M):
        return re.findall(pattern, Log.read(name), flags)

    @staticmethod
    def get_path(name=WORKER_LOG):
        return f'{option.temp_dir}/{name}'

    @staticmethod
    def open(name=WORKER_LOG, encoding='utf-8'):
    ...
    @staticmethod
    def set_pos(pos, name=WORKER_LOG):
        Log.pos[name] = pos

    @staticmethod
    def swap(name):
        pos = Log.pos.get(WORKER_LOG, 0)
        Log.pos[WORKER_LOG] = Log.pos.get(name, 0)
        Log.pos[name] = pos

    @staticmethod
    def wait_for_record(pattern, name=WORKER_LOG, wait=150, flags=re.M):
