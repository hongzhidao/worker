...
def test_tls_certificate_update():
    ...
    cert_old = client.get_server_certificate()

    client.certificate()

    assert cert_old != client.get_server_certificate(), 'update certificate'
...
def test_tls_certificate_change():
    ...
    assert cert_old != client.get_server_certificate(), 'change certificate'
...
def test_tls_certificate_chain_long(temp_dir):
    ...
    for i in range(chain_length - 1):
        if i == 0:
            ...
            ca(cert='int{}'.format(chain_length - 2), out='end')
        else:
            ca(cert='int{}'.format(i), out='int{}'.format(i + 1))
...
def test_tls_reconfigure():
    ...
    assert client.get(sock=sock)['status'] == 200, 'reconfigure status'
    assert client.get_ssl()['status'] == 200, 'reconfigure tls status'
