...
def test_static_space_in_name(temp_dir):
    ...
    assert (
        client.get(url='/ di r %2Ffi le')['body'] == 'blah'
    ), 'slash encoded'
    assert (
        client.get(url='/ di r /fi%20le')['body'] == 'blah'
    ), 'file encoded'
    assert (
    ...
    if utf8:
        ...
        assert (
            client.get(url='/ di r /фа йл')['body'] == 'blah'
        ), 'file name 2'
...
def test_static_mime_types():
    ...
    assert (
        client.get(url='/index.html')['headers']['Content-Type']
        == 'text/plain'
    ), 'mime_types html'
    assert (
    ...
    assert (
        client.get(url='/dir/file')['headers']['Content-Type']
        == 'text/plain'
